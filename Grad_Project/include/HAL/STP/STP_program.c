...
/* Lights exactly ONE column per call, then advances to the next.
 * On its own this still needs to be called at a steady rate by something -
 * see STP_StartAutoRefresh() below, which drives it from a hardware timer
 * interrupt instead of the cooperative OS task queue. */
