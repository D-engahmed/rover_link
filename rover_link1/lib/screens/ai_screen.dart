...
class _AiScreenState extends State<AiScreen> {
  ...
  void initState() {
    ...
    _traceSubscription = widget.commandService.traces.listen((value) {
      if (!mounted) return;
      setState(() { _traces.insert(0, value); if (_traces.length > 40) _traces.removeLast(); });
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final last = _traces.isEmpty ? null : _traces.first;
    ...
    return Scaffold(
      ...
      body: SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(16, 18, 16, 20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _card(title: 'ON-DEVICE NAVIGATION', child: Column(children: [
          Row(children: [Expanded(child: Text(aiRunning ? 'AI is controlling the rover locally.' : 'AI is ready. Connect the rover, then start autopilot.', style: const TextStyle(color: Color(0xFFB0BAC5), fontSize: 14))), const SizedBox(width: 12), ElevatedButton(onPressed: _busy ? null : _toggleAi, child: Text(aiRunning ? 'STOP' : 'START AI'))]),
          const SizedBox(height: 12), const Align(alignment: Alignment.centerLeft, child: Text('Model: rover_navigation_policy_v0.1 (embedded baseline)', style: TextStyle(color: Color(0xFF69F0AE), fontSize: 11))),
        ])),
        const SizedBox(height: 14),
        _card(title: 'AI COMMAND → HARDWARE', child: Column(children: [
          _row('AI decision', _decision?.label ?? '--', good: _decision != null),
          _row('Confidence', _decision == null ? '--' : '${(_decision!.confidence * 100).toStringAsFixed(0)}%'),
          _row('Last stage', last?.stageLabel ?? '--', good: last?.stage == CommandStage.sent),
          _row('Wire command', last?.wireCommand.isEmpty == false ? '${last!.wireCommand}\\r\\n' : '--'),
          _row('TX latency', last?.latencyMs == null ? '--' : '${last!.latencyMs} ms'),
        ...
        const SizedBox(height: 14),
        _card(title: 'DECISION SCORES', child: Column(children: [
          _row('Left', _decision == null ? '--' : _decision!.scoreLeft.toStringAsFixed(2)),
          _row('Forward', _decision == null ? '--' : _decision!.scoreForward.toStringAsFixed(2)),
          _row('Right', _decision == null ? '--' : _decision!.scoreRight.toStringAsFixed(2)),
        ])),
        const SizedBox(height: 14),
        _card(title: 'COMMAND TRACE', child: _traces.isEmpty ? const Text('No commands yet.', style: TextStyle(color: Color(0xFF9AA7B5))) : Column(children: _traces.take(12).map(_traceRow).toList())),
        const SizedBox(height: 14),
